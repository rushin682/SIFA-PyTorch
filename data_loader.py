...
def _decode_samples(image_list, shuffle=False):
    ...
    label_vol = tf.slice(label_vol, [1, 0, 0], label_size)

    batch_y = tf.one_hot(tf.cast(tf.squeeze(label_vol), tf.uint8), 5, axis=0)

    print("data_vol shape", tf.expand_dims(data_vol[1, :, :], axis=0).shape)
    print("label_vol shape", batch_y)

    return tf.expand_dims(data_vol[1, :, :], axis=0), batch_y
...
def load_data(source_pth, target_pth, do_shuffle=True):
    ...
    image_i = tf.concat((image_i,image_i,image_i), axis=0)
    image_j = tf.concat((image_j,image_j,image_j), axis=0)
